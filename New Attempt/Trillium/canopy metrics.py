...
def process_single_cell(args):
    ...
    idx, row, raster_folder, spatial_index, raster_info, grid_crs, intersection_method = args
    ...
    try:
        ...
        if not intersecting_rasters:
            return {
                'canopy_extent': 0,
                'morans_i': 0,
                # ... all other zero metrics
                'cell_id': cell_id,
                'num_intersecting_rasters': 0
            }

        # Mosaic/process rasters
        if intersection_method == 'mosaic' or len(intersecting_rasters) > 1:
            raster_data, transform = mosaic_rasters(
                intersecting_rasters, geometry, grid_crs
            )
        else:
            with rasterio.open(intersecting_rasters[0]) as src:
                out_image, transform = rasterio.mask.mask(
...
def process_grid_cells_with_raster_association_parallel(
        grid_gdf, raster_folder, aoi_size, output_dir='.',
        intersection_method='mosaic', n_workers=None
):
    ...
    args_list = [
        (idx, row, raster_folder, spatial_index, raster_info,
         grid_gdf.crs, intersection_method)
    ...
    with Pool(processes=n_workers) as pool:
        results = list(tqdm(
            pool.imap(process_single_cell, args_list),
            total=len(args_list),
...
if __name__ == "__main__":
    ...
    for grid_path in grid_paths:
        ...
        # Process grid cells
        results_df = process_grid_cells_with_raster_association_parallel(
            grid,
            raster_folder,
            aoi_size,
            output_directory,
            intersection_method='mosaic',
            n_workers=190  # Or whatever number you want
        )
